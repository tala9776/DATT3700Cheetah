...
void messageReceived(String topic, byte[] payload) {
  ...
  if (topic.equals("/LAB1CHEETAH/JAMES/PRESENCE")) {
...
void draw() { 
  ...
  if(newX == alexisX && newY == alexisY) {
    alexisPresent = false;
    client.publish("/LAB1CHEETAH/ALEXIS/PRESENCE", str(alexisPresent));
  } else {
    alexisX = newX;
    alexisY = newY;
    alexisPresent = true;
    client.publish("/LAB1CHEETAH/ALEXIS/PRESENCE", str(alexisPresent));
    client.publish("/LAB1CHEETAH/ALEXIS/X", str(alexisX));
    client.publish("/LAB1CHEETAH/ALEXIS/Y", str(alexisY));
  }
